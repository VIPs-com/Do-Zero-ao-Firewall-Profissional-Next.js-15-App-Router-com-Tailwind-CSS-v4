...
src/
  ...
  data/
    quizQuestions.ts        # perguntas do quiz — 215 perguntas (firewall=92, fundamentos=60, avancados=63; Sprint QUIZ-FUNDAMENTOS-DEPTH: +15 para profundidade da trilha Fundamentos)
...
- ✅ Sprint GLOSSARIO-V2: +15 termos atingindo 100 total — TOTP (RFC 6238 + PAM), PAM (pam_google_authenticator/pam_ldap), SMB/CIFS (Samba portas 137-445), DN (Distinguished Name LDAP), objectClass (inetOrgPerson+posixAccount), PFS (Diffie-Hellman efêmero VPN), XDP (eBPF no driver NIC), SLI (métrica SLO), Postmortem (blameless/Just Culture), Toil (SRE <50%), ACME (HTTP-01/DNS-01 Let's Encrypt), Cilium (CNI eBPF O(1)), Artifact+Runner (CI/CD), DHCP Relay; total 85→100.
