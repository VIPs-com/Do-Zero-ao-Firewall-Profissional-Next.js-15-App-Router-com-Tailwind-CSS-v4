...
- ✅ Sprint EXERCICIOS (4 rodadas — COMPLETO): seções "Exercícios Guiados" com 3 labs hands-on adicionadas a 19 módulos — Round 1: docker, hardening, audit-logs, dnat; Round 2: nginx-ssl, lan-proxy, port-knocking; Round 3: wan-nat, opnsense; Round 4: vpn-ipsec, dns, nextcloud, proxmox; Round 5 (final): ataques-avancados (recon nmap/SYN flood/ARP spoofing), cicd (pipeline CI/Docker push GHCR/deploy SSH), instalacao (interfaces/Netplan/IP forwarding), laboratorio (KVM/virsh/bridges), service-mesh (Istio install/canary VirtualService/Kiali+Jaeger), sre (SLOs recording rules/burn rate alerts/postmortem template). 100% de cobertura em todos os módulos. Sem mudanças em constantes.
