...
- ✅ Sprint EXERCICIOS (2 rodadas): seções "Exercícios Guiados" com 3 labs hands-on adicionadas a 7 módulos core — docker (bridge/iptables/internal network), hardening (SSH keygen/sysctl/Lynis), audit-logs (SSH monitor/auditd/forensics), dnat (PREROUTING/port-map/tcpdump dual), nginx-ssl (autoassinado/openssl/security-headers), lan-proxy (Squid/REDIRECT iptables/ACL blocklist), port-knocking (knockd/iptables-native/port-scan monitoring). Sem mudanças em constantes.
