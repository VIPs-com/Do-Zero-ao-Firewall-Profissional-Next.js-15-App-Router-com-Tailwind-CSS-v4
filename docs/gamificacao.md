...
- **Checkpoints** — 223 validações técnicas concluídas (ALL_CHECKLIST_IDS)
- **Quiz score** — 0–100, persiste em localStorage

---

## Tabela de badges (79 total)
...
| 🤿 | Mergulhador | `deep-diver` | Visitar todas as 80 páginas de conteúdo |
...
| **🥷** | **Linux Ninja** | **`linux-ninja`** | **≥ 167 checkpoints (75% dos 223)** ★ milestone |
...
| 👤 | Gestão de Usuários | `usuarios-master` | usuario-criado + grupo-criado + sudo-configurado |
| 🔎 | Troubleshooting Master | `troubleshooting-master` | trouble-conectividade + trouble-porta + trouble-logs |
| **🏁** | **Ground Zero** | **`ground-zero`** | **Completar os 17 módulos da Trilha Fundamentos Linux** ★ milestone |
| ⚖️ | HAProxy Master | `haproxy-master` | haproxy-instalado + haproxy-backend + haproxy-stats |
| 🛰️ | CrowdSec Master | `crowdsec-master` | crowdsec-instalado + crowdsec-cenarios + crowdsec-bouncer |
| 🔗 | Tailscale Master | `tailscale-master` | tailscale-instalado + tailscale-mesh + tailscale-subnet |
...
|------|--------|----------|
| **Comum** | 72 badges | Toast slide-in 4s (canto inferior direito, z-50) |
...
const MILESTONE_BADGES = new Set<BadgeId>([
  'course-master', 'advanced-master', 'quiz-master', 'linux-ninja',
  'sigma-master', 'certificado', 'ground-zero',
]);
...
## Checkpoints de validação (223 IDs — ALL_CHECKLIST_IDS)
...
[
  ...
  // Sprint I.23 — OPNsense / pfSense (3)
  'opnsense-instalado', 'opnsense-regras', 'opnsense-vpn',
  // Sprint I.24 — Nextcloud (3)
...
```typescript
// 75% de 223 checkpoints = 167 (floor)
