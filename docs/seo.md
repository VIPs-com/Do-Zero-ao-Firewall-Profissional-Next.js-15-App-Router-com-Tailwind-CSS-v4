...
Toda a configuração de metadata vive em **`src/lib/seo.ts`**. Isso é essencial para manter consistência: todas as 92 rotas do `ROUTE_SEO` apontam para o mesmo helper e aparecem automaticamente no sitemap (o build reporta ~97 rotas — inclui sitemap/robots/og/icon/apple-icon/manifest/_not-found).

## Arquivo central: `src/lib/seo.ts`

- `SITE_CONFIG` — nome do site, URL base, keywords globais, theme color, autor
- `ROUTE_SEO` — mapa `{ '/rota': { title, description } }` para as 92 rotas
