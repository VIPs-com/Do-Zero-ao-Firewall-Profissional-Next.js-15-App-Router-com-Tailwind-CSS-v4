...
Toda a configuração de metadata vive em **`src/lib/seo.ts`**. Isso é essencial para manter consistência: todas as 88 rotas apontam para o mesmo helper e aparecem automaticamente no sitemap.

## Arquivo central: `src/lib/seo.ts`

- `SITE_CONFIG` — nome do site, URL base, keywords globais, theme color, autor
- `ROUTE_SEO` — mapa `{ '/rota': { title, description } }` para as 88 rotas
