...
const GLOSSARY: Term[] = [
  ...
  // Sprint GLOSSARIO-V2 — +15 termos para cobertura 85→100
  { term: "TOTP", definition: "Time-based One-Time Password — código de 6 dígitos baseado no tempo atual + segredo compartilhado (RFC 6238). Muda a cada 30s. Implementado no SSH via libpam-google-authenticator + /etc/pam.d/sshd.", category: "Segurança", icon: <Shield size={14} /> },
  { term: "PAM", definition: "Pluggable Authentication Modules — framework do Linux que insere módulos de autenticação em serviços sem modificar o código. pam_google_authenticator.so adiciona TOTP ao SSH; pam_ldap.so autentica via LDAP.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "SMB / CIFS", definition: "Server Message Block / Common Internet File System — protocolo de compartilhamento de arquivos da Microsoft, implementado no Linux pelo Samba. Portas 137-139 (NetBIOS) e 445 (SMB direto). SMB3 inclui criptografia end-to-end.", category: "Redes", icon: <Globe size={14} /> },
  { term: "DN (LDAP)", definition: "Distinguished Name — identificador único de uma entrada no diretório LDAP. Forma: uid=joao,ou=usuarios,dc=workshop,dc=local. Lê-se da folha para a raiz da árvore DIT.", category: "Segurança", icon: <Shield size={14} /> },
  { term: "objectClass (LDAP)", definition: "Atributo LDAP que define o schema de uma entrada. inetOrgPerson fornece cn/sn/mail. posixAccount adiciona uid/gidNumber/homeDirectory para login Unix. shadowAccount gerencia expiração de senhas.", category: "Segurança", icon: <Shield size={14} /> },
  { term: "PFS", definition: "Perfect Forward Secrecy — propriedade de uma sessão criptografada onde a chave de sessão não pode ser derivada da chave privada do servidor. Cada sessão usa par Diffie-Hellman efêmero descartado após uso. Ativado com pfs=yes no StrongSwan.", category: "VPN", icon: <Zap size={14} /> },
  { term: "XDP", definition: "eXpress Data Path — gancho eBPF executado no driver de rede antes do stack TCP/IP do kernel processar o pacote. XDP_DROP descarta em nanosegundos — defesa DDoS sem custo de processamento do kernel. Cilium usa XDP para load balancing de alta performance.", category: "Evolução", icon: <Activity size={14} /> },
  { term: "SLI", definition: "Service Level Indicator — métrica específica que quantifica a experiência do usuário. Exemplos: taxa de sucesso de requisições (disponibilidade), latência P99 (performance), taxa de erro (qualidade). O SLO define o alvo para o SLI.", category: "Observabilidade", icon: <Activity size={14} /> },
  { term: "Postmortem", definition: "Documento blameless que analisa um incidente após sua resolução. Estrutura: resumo → timeline → causa raiz → fatores contribuintes → ações corretivas. Foca em falhas de sistema, não em culpar indivíduos (Just Culture).", category: "Observabilidade", icon: <Activity size={14} /> },
  { term: "Toil (SRE)", definition: "Trabalho operacional manual, repetitivo e automável que escala linearmente com o crescimento do sistema. Exemplos: reiniciar serviço manualmente, processar tickets repetitivos. Google SRE mantém toil abaixo de 50% do tempo de trabalho.", category: "Observabilidade", icon: <Activity size={14} /> },
  { term: "ACME", definition: "Automatic Certificate Management Environment — protocolo RFC 8555 usado pelo Let's Encrypt. Prova posse do domínio via HTTP-01 (arquivo em /.well-known/), DNS-01 (registro TXT) ou TLS-ALPN-01. Certbot e Traefik implementam ACME nativamente.", category: "Segurança", icon: <Lock size={14} /> },
  { term: "Cilium", definition: "CNI (Container Network Interface) baseado em eBPF para Kubernetes. Substitui kube-proxy usando eBPF maps — O(1) em vez de O(N) iptables. Fornece NetworkPolicy L7 (HTTP path/method), load balancing DSR e Hubble para observabilidade.", category: "Kubernetes", icon: <Network size={14} /> },
  { term: "Artifact (CI/CD)", definition: "Arquivo ou conjunto de arquivos produzido por um job de CI/CD e armazenado para uso posterior. Ex: relatório de cobertura de testes, binário compilado, imagem Docker. No GitHub Actions: actions/upload-artifact / download-artifact.", category: "DevOps & IaC", icon: <Zap size={14} /> },
  { term: "Runner (CI/CD)", definition: "Máquina que executa jobs de um pipeline CI/CD. GitHub-hosted runners são VMs efêmeras (Ubuntu/Windows/macOS) provisionadas pelo GitHub. Self-hosted runners são servidores próprios — mais controle, acesso à rede interna, zero custo de minutos.", category: "DevOps & IaC", icon: <Zap size={14} /> },
  { term: "DHCP Relay", definition: "Agente que encaminha broadcasts DHCP (UDP porta 67/68) entre VLANs diferentes. Necessário quando o servidor DHCP está em uma rede separada dos clientes. Configurado no roteador ou switch L3 com ip helper-address (Cisco) ou dhcp-relay.", category: "Redes", icon: <Globe size={14} /> },

  // Sprint GLOSSARIO-V3 — +15 termos avançados
  { term: "Idempotência", definition: "Propriedade de uma operação que produz o mesmo resultado independente de quantas vezes é executada. No Ansible: um módulo idempotente verifica o estado atual antes de agir — se já está correto, retorna 'ok' sem mudanças. Oposto: módulo shell/command que sempre executa e retorna 'changed'.", category: "DevOps & IaC", icon: <Zap size={14} /> },
  { term: "Handler (Ansible)", definition: "Task especial que só executa quando notificada por outra task via 'notify'. Acumula notificações e executa UMA ÚNICA VEZ ao final do play — mesmo que 10 tasks notifiquem o mesmo handler. Exemplo: restart nginx após modificar nginx.conf, sites-available e SSL — só 1 restart.", category: "DevOps & IaC", icon: <Zap size={14} /> },
  { term: "eBPF Verifier", definition: "Componente do kernel Linux que analisa estaticamente todo programa eBPF antes de carregá-lo. Garante: sem loops infinitos (bounded loops), sem acesso fora dos limites de memória, calls apenas para helpers permitidos. Sem o verifier, programas eBPF com bug travariam o kernel.", category: "Kernel", icon: <Shield size={14} /> },
  { term: "BPF Map", definition: "Estrutura de dados no kernel Linux acessível tanto por programas eBPF quanto por userspace via syscall bpf(). Tipos principais: BPF_MAP_TYPE_HASH (O(1)), ARRAY (índice direto), LRU_HASH (eviction automático), RINGBUF (eventos eficientes). Cilium usa maps para substituir iptables com O(1) lookup.", category: "Kernel", icon: <Activity size={14} /> },
  { term: "CrashLoopBackOff", definition: "Estado de Pod no Kubernetes onde o container inicia, falha rapidamente e o K8s reinicia repetidamente com backoff exponencial (10s → 20s → 40s → 5min). Causas comuns: configuração errada, dependência indisponível, OOMKill. Diagnosticar com: kubectl logs pod-name --previous.", category: "Kubernetes", icon: <Network size={14} /> },
  { term: "ImagePullBackOff", definition: "Erro de Pod Kubernetes onde o kubelet não consegue baixar a imagem Docker. Causas: imagem não existe, tag errada, credenciais ausentes para registry privado. Solução: verificar imagePullSecrets no manifesto ou kubectl create secret docker-registry.", category: "Kubernetes", icon: <Network size={14} /> },
  { term: "ForceNew (Terraform)", definition: "Atributo de um campo no provider Terraform que, quando modificado, força a destruição e recriação do recurso (em vez de update in-place). Exemplos no AWS: trocar a AMI de uma EC2, mudar a subnet. O terraform plan mostra '# must be replaced' quando ForceNew é ativado.", category: "DevOps & IaC", icon: <Terminal size={14} /> },
  { term: "Concurrency (GitHub Actions)", definition: "Configuração que agrupa execuções de workflow pelo mesmo grupo e define comportamento quando um novo disparo ocorre: cancel-in-progress: true cancela o anterior (ideal para staging); false coloca na fila (ideal para produção). Previne race conditions em deployments.", category: "DevOps & IaC", icon: <Zap size={14} /> },
  { term: "mTLS (Mutual TLS)", definition: "Variante do TLS onde AMBOS os lados (cliente e servidor) apresentam certificados para autenticação mútua. Padrão no Istio Service Mesh — Pods se autenticam via SPIFFE X.509. Diferente do TLS normal onde só o servidor tem certificado.", category: "Segurança", icon: <Lock size={14} /> },
  { term: "SPIFFE", definition: "Secure Production Identity Framework for Everyone — framework de identidade para workloads em ambientes dinâmicos (containers, cloud). Cada serviço recebe um ID único no formato spiffe://trust-domain/path. Istio usa SPIFFE/SPIRE para emitir certificados X.509 a cada Pod automaticamente.", category: "Kubernetes", icon: <Shield size={14} /> },
  { term: "VirtualService (Istio)", definition: "Recurso do Istio que define regras de roteamento de tráfego para serviços — onde o tráfego vai e como. Permite: canary deployment (90%/10% por peso), roteamento por header (A/B testing), retry automático, timeout, injeção de falhas para chaos engineering.", category: "Observabilidade", icon: <Network size={14} /> },
  { term: "Envoy Proxy", definition: "Proxy L7 de alto desempenho escrito em C++, criado pela Lyft e doado à CNCF. É o sidecar do Istio — injetado automaticamente em cada Pod para interceptar todo tráfego, aplicar mTLS, coletar telemetria e executar VirtualService rules. Também é o data plane do Consul e do AWS App Mesh.", category: "Observabilidade", icon: <Server size={14} /> },
  { term: "PodDisruptionBudget", definition: "Objeto Kubernetes (PDB) que limita quantos Pods de um deployment podem ser indisponíveis simultaneamente durante manutenções voluntárias (drain de nó, upgrade de cluster). Exemplo: minAvailable: 2 garante que kubectl drain nunca derruba mais do que mantém 2 rodando.", category: "Kubernetes", icon: <Shield size={14} /> },
  { term: "Recording Rule (Prometheus)", definition: "Expressão PromQL pré-calculada que o Prometheus avalia periodicamente e armazena como nova série temporal. Benefício: queries complexas sobre milhões de amostras ficam instantâneas pois o resultado já está computado. Regra de ouro: se uma query leva >1s no dashboard, crie uma recording rule.", category: "Observabilidade", icon: <Activity size={14} /> },
  ...
  // ── Sprint GLOSSARIO-SHELL — comandos essenciais e produtividade no terminal ─
  { term: "grep", definition: "Filtra linhas que casam com um padrão. 'grep ERROR /var/log/syslog' lista erros; '-i' ignora case; '-v' inverte (linhas que NÃO casam); '-r' recursivo em diretório; '-E' habilita regex estendida. Costuma vir no meio de pipes ('| grep ...').", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "awk", definition: "Linguagem de processamento de texto orientada a colunas. Trata cada linha como campos ($1, $2, $NF) e processa em STREAMING — sem carregar o arquivo na RAM. Ideal para logs gigantes (multi-GB): 'awk {print $1} access.log | sort | uniq -c | sort -rn | head' = top IPs. Soma, agrupa, formata num único passe.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "sed", definition: "Stream editor — aplica substituições/edições em fluxo. 'sed -i.bak s/yes/no/ arq.conf' troca in-place com backup. Usado em scripts e Ansible para alterar configs sem abrir editor. Sempre use '-i.bak' em produção para gerar uma cópia de segurança.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "xargs", definition: "Pega a saída de um comando e a injeta como ARGUMENTOS do próximo. Útil quando o comando alvo não aceita stdin. 'find . -name *.tmp -print0 | xargs -0 rm' deleta com segurança. Use '-P N' para rodar em paralelo (N processos simultâneos).", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "find", definition: "Busca arquivos no sistema por nome, tamanho, idade, permissão ou dono. 'find /var -name *.log -mtime +30 -size +100M' acha logs antigos e grandes. '-exec ... {} ;' executa um comando em cada resultado; combinado com xargs ganha paralelismo.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "pipe (|)", definition: "Operador do shell que conecta a stdout de um comando à stdin do próximo. 'comando1 | comando2 | comando3' forma um pipeline — base da filosofia Unix (cada programa faz uma coisa, e bem). Sem pipe, não há shell.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "stdin / stdout / stderr", definition: "Os três canais padrão de um processo: 0 = stdin (entrada), 1 = stdout (saída normal), 2 = stderr (erros). Redirecionar: '> arquivo' (stdout), '2> erros.log' (stderr), '2>&1' (junta stderr no stdout), '> /dev/null 2>&1' (descarta tudo).", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "alias", definition: "Apelido do shell — substitui um nome curto por uma linha de comando. 'alias ll=ls -lah' gravado no ~/.bashrc fica permanente. Não aceita argumentos no meio; quando precisar disso, use FUNÇÃO (ex.: 'mkcd(){ mkdir -p $1 && cd $1; }').", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "~/.bashrc", definition: "Script lido pelo Bash em toda sessão interativa. É onde vivem aliases, funções, exports (PATH, HISTSIZE) e prompt customizado (PS1). 'source ~/.bashrc' aplica mudanças sem reabrir o terminal. Equivalente no PowerShell: $PROFILE.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "history", definition: "Lista os últimos comandos digitados (guardados em ~/.bash_history). 'history | grep iptables' acha aquela regra antiga. '!1234' re-executa a linha; '!!' repete o último; Ctrl+R faz reverse-search interativa. HISTCONTROL=ignoreboth + linha-com-espaço evitam gravar comandos sensíveis.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "PATH", definition: "Variável de ambiente que lista as pastas onde o shell procura executáveis. 'echo $PATH' mostra; quando você digita 'ls', o Bash varre o PATH na ordem e roda o primeiro encontrado. 'export PATH=$HOME/bin:$PATH' no ~/.bashrc estende a busca para sua pasta pessoal.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "sudo", definition: "Executa um comando como outro usuário (padrão: root). Lê regras de /etc/sudoers (edite SEMPRE com 'visudo'). 'sudo -i' abre shell root; 'sudo -u www-data ...' roda como o usuário do Nginx. Logs vão em /var/log/auth.log.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "chmod", definition: "Muda permissões de arquivos/diretórios. Octal: 'chmod 644 arq' (rw-r--r--), 'chmod 755 dir' (rwxr-xr-x). Simbólico: 'chmod +x script.sh' (executável), 'chmod g-w arq' (remove write do grupo). Permissão default é controlada pelo umask.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "chown", definition: "Muda dono e grupo de arquivos. 'sudo chown -R www-data:www-data /var/www' deixa a pasta inteira pertencer ao usuário/grupo do Nginx. Combinado com chmod, é a primeira coisa a checar quando um serviço não consegue ler/escrever onde deveria.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "tar", definition: "Arquivador clássico do Unix — agrupa muitos arquivos num único .tar (sem compactar) ou .tar.gz/.tar.bz2/.tar.xz (compactado). 'tar czf backup.tar.gz /etc' empacota; 'tar xzf backup.tar.gz' extrai. Mnemônico: 'czf' = Create giZipped File, 'xzf' = eXtract giZipped File.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "rsync", definition: "Sincroniza arquivos entre locais (mesma máquina ou remoto via SSH) transferindo apenas as DIFERENÇAS. 'rsync -avz --delete /src/ user@host:/dest/' espelha respeitando timestamps e permissões. Resume transferências interrompidas com '--partial'. Pilar de backups 3-2-1.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "ln -s (symlink)", definition: "Cria um link simbólico — atalho que aponta para outro path. 'ln -s /opt/app/current /etc/nginx/sites-enabled/site' deixa a config viva no /opt mas referenciada onde o Nginx espera. Difere do hard link (ln sem -s) que cria outro nome para o mesmo inode.", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "inode", definition: "Estrutura no filesystem que guarda os metadados de cada arquivo (permissões, dono, tamanho, blocos de dados) — exceto o NOME, que vive na pasta. 'df -i' mostra inodes em uso. Disco pode lotar de inodes mesmo com espaço livre (muitos arquivos pequenos).", category: "Sistema", icon: <Terminal size={14} /> },
  { term: "ps / top / htop", definition: "Visualizam processos. 'ps aux' lista TODOS com CPU/RAM/comando; 'ps auxf' mostra árvore. 'top' atualiza em tempo real (q sai); 'htop' é a versão colorida e navegável. Combine com awk para somar memória por usuário: 'ps aux | awk NR>1{m[$1]+=$6}END{for(u in m)print u,m[u]/1024,MB}'.", category: "Diagnóstico", icon: <Search size={14} /> },
  { term: "ss / netstat", definition: "Mostram sockets/portas em uso. 'ss -tulpn' (ou 'netstat -tulpn' no legado): -t TCP, -u UDP, -l listening, -p processo, -n números (sem DNS). É a primeira coisa a rodar quando 'serviço X está rodando? em que porta?'. ss é mais rápido que netstat em sistemas modernos.", category: "Diagnóstico", icon: <Search size={14} /> },
  { term: "dig", definition: "Cliente DNS profissional. 'dig exemplo.com' resolve A; 'dig MX exemplo.com' lista servidores de e-mail; 'dig +short @8.8.8.8 exemplo.com' usa o Google como resolver. '+trace' segue a delegação recursiva da raiz até a zona — essencial para diagnóstico DNS.", category: "Diagnóstico", icon: <Search size={14} /> },
  { term: "curl / wget", definition: "Clientes HTTP de linha de comando. 'curl -sSI https://x' pega só os headers; '-X POST -d json' envia POST; '-o arq' grava resposta. wget é mais focado em download recursivo de mirrors. Substitutos: 'httpie' (curl mais legível), 'aria2c' (downloads paralelos).", category: "Diagnóstico", icon: <Search size={14} /> },
];
