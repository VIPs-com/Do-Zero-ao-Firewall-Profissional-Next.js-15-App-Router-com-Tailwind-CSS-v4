 ...
 *   totalTopics = 98          (Sprint PILARES: +C10-C12)
 *   checklistItemsCount = 199 (Sprint PILARES: +9 checkpoints)
 *   BADGE_DEFS tem 71 chaves  (Sprint PILARES: +storage/dba/mail-master)
 *
...
test('dashboard exibe contadores de checklist e quiz corretamente', async ({ page }) => {
  ...
  // Labs Concluídos: 3/199 (seed exato — não muda com o load)
...
test('dashboard exibe 0/71 badges para usuário sem progresso', async ({ page }) => {
  // Sem seed — estado completamente limpo (fixture já limpou)
  await page.goto('/dashboard');
  await page.waitForLoadState('networkidle');

  // Sem nenhum badge desbloqueado — 71 badges total (Sprint PILARES)
