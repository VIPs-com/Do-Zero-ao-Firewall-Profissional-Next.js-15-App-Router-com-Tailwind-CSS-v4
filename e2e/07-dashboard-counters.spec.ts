 ...
 *   checklistItemsCount = 214 (Sprint CLOUD/GIT/CARREIRA: +9 checkpoints)
 *   BADGE_DEFS tem 76 chaves  (Sprint CLOUD/GIT/CARREIRA: +cloud/git/carreira-master)
 *
...
test('dashboard exibe contadores de checklist e quiz corretamente', async ({ page }) => {
  ...
  // Labs Concluídos: 3/214 (seed exato — não muda com o load)
  await expect(page.getByText('3/214', { exact: true })).toBeVisible();

  // Melhor Quiz: 75% — exact evita colidir com "Complete 75% do checklist (...)"
  await expect(page.getByText('75%', { exact: true })).toBeVisible();
});

test('dashboard exibe 0/76 badges para usuário sem progresso', async ({ page }) => {
  // Sem seed — estado completamente limpo (fixture já limpou)
  await page.goto('/dashboard');
  await page.waitForLoadState('networkidle');

  // Sem nenhum badge desbloqueado — 76 badges total (Sprint CLOUD/GIT/CARREIRA)
