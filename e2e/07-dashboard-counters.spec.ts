 ...
 *   totalTopics = 77          (Sprint I.20: +1 Service Mesh)
 *   checklistItemsCount = 136 (Sprint I.20: +3 checkpoints service-mesh)
 *   BADGE_DEFS tem 49 chaves  (Sprint I.20: +service-mesh-master)
 *
...
test('dashboard exibe contadores de checklist e quiz corretamente', async ({ page }) => {
  ...
  // Labs Concluídos: 3/136 (seed exato — não muda com o load)
  await expect(page.getByText('3/136')).toBeVisible();

  // Melhor Quiz: 75%
  await expect(page.getByText('75%')).toBeVisible();
});

test('dashboard exibe 0/49 badges para usuário sem progresso', async ({ page }) => {
