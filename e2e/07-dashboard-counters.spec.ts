 ...
 *   totalTopics = 82          (Sprint I.25: +1 eBPF Avançado)
 *   checklistItemsCount = 151 (Sprint I.25: +3 checkpoints ebpf-avancado)
 *   BADGE_DEFS tem 54 chaves  (Sprint I.25: +ebpf-avancado-master)
 *
...
test('dashboard exibe contadores de checklist e quiz corretamente', async ({ page }) => {
  ...
  // Labs Concluídos: 3/151 (seed exato — não muda com o load)
  await expect(page.getByText('3/151')).toBeVisible();

  // Melhor Quiz: 75%
  await expect(page.getByText('75%')).toBeVisible();
});

test('dashboard exibe 0/54 badges para usuário sem progresso', async ({ page }) => {
  // Sem seed — estado completamente limpo (fixture já limpou)
  await page.goto('/dashboard');
  await page.waitForLoadState('networkidle');

  // Sem nenhum badge desbloqueado (visitedPages << 5, sem quiz)
  await expect(page.getByText('0/54')).toBeVisible();
});
