 ...
 *   totalTopics = 100         (Sprint REDES-L23: +C13/C14)
 *   checklistItemsCount = 205 (Sprint REDES-L23: +6 checkpoints)
 *   BADGE_DEFS tem 73 chaves  (Sprint REDES-L23: +redes/ha-master)
 *
...
test('dashboard exibe contadores de checklist e quiz corretamente', async ({ page }) => {
  ...
  // Labs Concluídos: 3/205 (seed exato — não muda com o load)
  await expect(page.getByText('3/205', { exact: true })).toBeVisible();

  // Melhor Quiz: 75% — exact evita colidir com "Complete 75% do checklist (...)"
  await expect(page.getByText('75%', { exact: true })).toBeVisible();
});

test('dashboard exibe 0/73 badges para usuário sem progresso', async ({ page }) => {
  // Sem seed — estado completamente limpo (fixture já limpou)
  await page.goto('/dashboard');
  await page.waitForLoadState('networkidle');

  // Sem nenhum badge desbloqueado — 73 badges total (Sprint REDES-L23)
