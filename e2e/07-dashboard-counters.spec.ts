 ...
 *   totalTopics = 94          (Sprint FORTALEZA: +C06-C08)
 *   checklistItemsCount = 187 (Sprint FORTALEZA: +9 checkpoints)
 *   BADGE_DEFS tem 67 chaves  (Sprint CÓDICE: +incident-master)
 *
...
test('dashboard exibe contadores de checklist e quiz corretamente', async ({ page }) => {
  ...
  // Labs Concluídos: 3/187 (seed exato — não muda com o load)
  await expect(page.getByText('3/187', { exact: true })).toBeVisible();

  // Melhor Quiz: 75% — exact evita colidir com "Complete 75% do checklist (...)"
  await expect(page.getByText('75%', { exact: true })).toBeVisible();
});

test('dashboard exibe 0/67 badges para usuário sem progresso', async ({ page }) => {
  // Sem seed — estado completamente limpo (fixture já limpou)
  await page.goto('/dashboard');
  await page.waitForLoadState('networkidle');

  // Sem nenhum badge desbloqueado — 67 badges total (Sprint FORTALEZA)
