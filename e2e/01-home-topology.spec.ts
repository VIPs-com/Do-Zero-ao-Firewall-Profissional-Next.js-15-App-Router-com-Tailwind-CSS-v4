 ...
 *   <svg role="group" aria-labelledby="topology-title">
 *     <title id="topology-title">Topologia de rede do laboratório Workshop Linux</title>
 *
 * Nota: role="group" (não "img") porque o SVG contém nós interativos focáveis
 * (role="button" tabIndex=0) — role="img" violaria nested-interactive (Sprint A11Y).
 */
test('home carrega e topologia SVG renderiza com título acessível', async ({ page }) => {
  ...
  const svg = page.locator('svg[role="group"][aria-labelledby="topology-title"]');
