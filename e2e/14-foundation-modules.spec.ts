 ...
 *   3. Checkpoint 'usuario-criado' contabilizado no dashboard (1/178)
...
test('checkpoint usuario-criado é contabilizado no dashboard (1/178)', async ({ page }) => {
...
test('checkpoint trouble-conectividade é contabilizado no dashboard', async ({ page }) => {
  ...
  await expect(page.getByText('2/178', { exact: true })).toBeVisible();
});
