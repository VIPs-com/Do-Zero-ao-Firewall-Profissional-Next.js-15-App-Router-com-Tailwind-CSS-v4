 ...
 *   3. Checkpoint 'usuario-criado' contabilizado no dashboard (1/214)
 ...
 *
 * checklistItemsCount = 178 (Sprint CÓDICE)
...
test('checkpoint usuario-criado é contabilizado no dashboard (1/214)', async ({ page }) => {
  ...
  await expect(page.getByText('1/214', { exact: true })).toBeVisible();
});
...
test('checkpoint trouble-conectividade é contabilizado no dashboard', async ({ page }) => {
  ...
  await expect(page.getByText('2/214', { exact: true })).toBeVisible();
});
