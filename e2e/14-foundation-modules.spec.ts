 ...
 *   3. Checkpoint 'usuario-criado' contabilizado no dashboard (1/190)
...
test('checkpoint usuario-criado é contabilizado no dashboard (1/190)', async ({ page }) => {
...
test('checkpoint trouble-conectividade é contabilizado no dashboard', async ({ page }) => {
  ...
  await expect(page.getByText('2/190', { exact: true })).toBeVisible();
});
