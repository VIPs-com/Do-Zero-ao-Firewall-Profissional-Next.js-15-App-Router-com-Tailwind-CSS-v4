 ...
 *
 * checklistPercentage = round(valores true no checklist / 205 * 100) — conta
 * QUALQUER valor true (BadgeContext.checklistPercentage), então seedar 190
 * chaves true resulta em ~93%.
 */

/**
 * Seed que satisfaz isReady (190/205 ≈ 93% + quiz score).
 *
...
async function seedReady(page: import('@playwright/test').Page, quizScore: number) {
  await page.addInitScript((score) => {
    ...
    for (let i = 0; i < 190; i++) checklist[`chk-${i}`] = true;
