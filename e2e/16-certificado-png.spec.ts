 ...
 *
 * checklistPercentage = round(valores true no checklist / 187 * 100) — conta
 * QUALQUER valor true (BadgeContext.checklistPercentage), então seedar 170
 * chaves true resulta em ~91%.
 */

/**
 * Seed que satisfaz isReady (170/187 ≈ 91% + quiz score).
 *
...
async function seedReady(page: import('@playwright/test').Page, quizScore: number) {
  await page.addInitScript((score) => {
    ...
    for (let i = 0; i < 170; i++) checklist[`chk-${i}`] = true;
