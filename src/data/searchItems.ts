...
export const SEARCH_ITEMS: SearchItem[] = [
  ...
  { id: 'g-swap-conceito',       title: 'SWAP e Memória Virtual',
    description: 'O que é SWAP, OOM Killer, fallocate + mkswap + swapon — memória virtual no Linux.',
    category: 'Tópico', href: '/discos', icon: Server },
  { id: 'g-swap-swappiness',     title: 'vm.swappiness e tuning de SWAP',
    description: 'vm.swappiness=10 para servidores, quando não usar SWAP (K8s, NVMe, bancos de dados).',
    category: 'Tópico', href: '/discos', icon: Server },
  { id: 'g-swap-ram-monitoring', title: 'free -h e vmstat — monitorar RAM',
    description: 'free -h, vmstat si/so, /proc/meminfo — interpretar uso de memória e thrashing.',
    category: 'Tópico', href: '/discos', icon: Server },
  { id: 'g-filesystems',         title: 'ext4 vs xfs vs btrfs',
    description: 'Comparação de filesystems Linux: snapshots, redimensionar, checksums, quando usar cada.',
    category: 'Tópico', href: '/discos', icon: Server },
  { id: 'g-mkfs',                title: 'mkfs — formatar filesystem',
    description: 'mkfs.ext4, mkfs.xfs, mkfs.btrfs — escolher e gravar o sistema de arquivos na partição.',
    category: 'Tópico', href: '/discos', icon: Server },
  { id: 'g-quotas',              title: 'Quotas de disco — edquota',
    description: 'usrquota no fstab, quotacheck, quotaon, edquota, setquota, repquota — limite por usuário.',
    category: 'Tópico', href: '/discos', icon: Server },
  ...
  { id: 'g-luks-midias', category: 'Tópico', title: 'LUKS em NVMe, SATA, USB e SD', description: 'Identificar o device com lsblk -o ...,TRAN; a regra do «p» (nvme0n1p1, mmcblk0p1 vs sda1/sdb1); disco fixo (crypttab) vs removível; --sector-size 4096 e TRIM', href: '/seguranca-avancada', icon: Lock },
  { id: 'g-luks-pendrive', category: 'Tópico', title: 'Criptografar pen drive com LUKS', description: 'Pen drive ou cartão SD criptografado e portátil: luksFormat --type luks2, mkfs, abrir sob demanda com udisksctl unlock; interop (VeraCrypt, dislocker)', href: '/seguranca-avancada', icon: Lock },
  ...
  { id: 'g-awk',            category: 'Comando',   title: 'awk — Processamento avançado de logs gigantes',          description: 'Trata cada linha como campos ($1, $2, $NF) em STREAMING (sem carregar arquivo na RAM). Top IPs no access.log: awk {print $1} access.log | sort | uniq -c | sort -rn | head. Soma, agrupa e formata em logs de muitos GB num único passe', href: '/comandos#awk', icon: Terminal },
  { id: 'g-sed-shell',      category: 'Comando',   title: 'sed — Substituição não-interativa (in-place)',           description: 'Stream editor para alterar arquivos em script. sed -i.bak s/yes/no/ /etc/ssh/sshd_config troca com backup. -e encadeia múltiplas substituições. Pilar de automação Ansible que altera configs sem editor', href: '/comandos#sed', icon: Terminal },
  { id: 'g-xargs',          category: 'Comando',   title: 'xargs — Saída de um comando como argumento de outro',    description: 'find . -name *.tmp -print0 | xargs -0 rm deleta com segurança. -P N roda em paralelo (N processos). Resolve a limitação de comandos que não aceitam stdin (rm, kill, mkdir).', href: '/comandos#xargs', icon: Terminal },
  { id: 'g-aliases-bashrc', category: 'Comando',   title: 'Aliases e funções no ~/.bashrc',                         description: 'alias ll=ls -lah cria apelido permanente. Para argumentos use função: mkcd(){ mkdir -p $1 && cd $1; }. source ~/.bashrc aplica sem reabrir terminal. Ganha horas/mês de digitação', href: '/comandos#aliases', icon: Terminal },
  { id: 'g-history-bash',   category: 'Comando',   title: 'history — Reverse-search e re-execução',                 description: 'history | grep iptables acha comando antigo. !1234 re-executa linha; !! repete o último; sudo !! repete com sudo. Ctrl+R faz reverse-search interativa. HISTCONTROL=ignoreboth evita gravar duplicados/sensíveis', href: '/comandos#historico', icon: Terminal },
  { id: 'g-pipe-stdin',     category: 'Glossário', title: 'Pipe (|) e stdin/stdout/stderr',                         description: 'O pipe | conecta stdout de um comando à stdin do próximo — base da filosofia Unix. Redirecionamentos: > arquivo (stdout), 2> erros (stderr), 2>&1 (junta), > /dev/null 2>&1 (descarta tudo)', href: '/glossario', icon: Terminal },
  { id: 'g-path-env',       category: 'Glossário', title: 'PATH — onde o shell procura executáveis',                description: 'echo $PATH lista pastas (ordem importa). export PATH=$HOME/bin:$PATH no ~/.bashrc estende. Sem PATH, todo comando precisaria do path absoluto: /usr/bin/ls em vez de só ls', href: '/glossario', icon: Terminal },
  { id: 'g-chmod-chown',    category: 'Comando',   title: 'chmod e chown — Permissões e dono dos arquivos',         description: 'chmod 644 arq (rw-r--r--), chmod 755 dir, chmod +x script.sh. chown -R www-data:www-data /var/www transfere propriedade. Primeira coisa a checar quando um serviço não consegue ler/escrever', href: '/glossario', icon: Terminal },
  { id: 'g-rsync-shell',    category: 'Comando',   title: 'rsync — Sincronização incremental local/remota',         description: 'rsync -avz --delete /src/ user@host:/dest/ espelha respeitando timestamps. Transfere só DIFERENÇAS — pilar de backups 3-2-1. --partial resume transferências interrompidas', href: '/glossario', icon: Terminal },
  { id: 'g-tar',            category: 'Comando',   title: 'tar — Arquivamento e compactação',                       description: 'tar czf backup.tar.gz /etc empacota (Create giZipped File); tar xzf backup.tar.gz extrai (eXtract). Variantes: .tar.bz2 (j), .tar.xz (J). Combina muitos arquivos em um único — base de qualquer backup', href: '/glossario', icon: Terminal },
