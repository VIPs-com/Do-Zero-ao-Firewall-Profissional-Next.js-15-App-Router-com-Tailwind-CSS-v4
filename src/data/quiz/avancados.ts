...
export const AVANCADOS_QUESTIONS: QuizQuestion[] = [
  ...
  // ── LVM, RAID & Armazenamento (Sprint PILARES) ─────────────────────────────
  {
      text: 'Na hierarquia do LVM, qual é a ordem correta dos três conceitos?',
      badge: '💽 Storage',
      options: [
      ...
      correct: 1,
      explanation: 'O disco (ou partição) vira um Physical Volume com `pvcreate`. Vários PVs são reunidos num Volume Group (`vgcreate`) — um pool de espaço. Desse pool você fatia Logical Volumes (`lvcreate`), que são as partições flexíveis e redimensionáveis a quente.',
      trail: 'avancados',
    },
  {
      text: 'Qual nível de RAID por software espelha os dados, sobrevivendo à perda de um disco sem paridade?',
      badge: '💽 Storage',
      options: [
        'RAID 0 (striping)',
        'RAID 1 (espelhamento)',
      ...
      correct: 1,
      explanation: 'RAID 1 grava os mesmos dados em dois (ou mais) discos — espelhamento puro. Se um disco falha, o outro tem uma cópia íntegra. RAID 0 só faz striping (zero redundância); RAID 5/6 usam paridade. Cria-se com `mdadm --create --level=1`.',
      trail: 'avancados',
    },
  {
      text: 'Por que um snapshot de LVM é útil antes de uma operação arriscada?',
      badge: '💽 Storage',
      options: [
        'Ele duplica fisicamente todo o volume imediatamente',
        'Ele congela um estado do volume — se algo der errado, você restaura para aquele ponto',
        'Ele aumenta a velocidade de leitura do volume',
        'Ele converte o volume para ZFS automaticamente',
      ],
      correct: 1,
      explanation: 'Um snapshot LVM captura o estado do LV num instante usando copy-on-write — só os blocos que mudam depois consomem espaço. Antes de um upgrade ou migração, você cria o snapshot; se falhar, faz merge de volta e desfaz tudo.',
      trail: 'avancados',
    },
  {
      text: 'Qual recurso do ZFS detecta e corrige silenciosamente a corrupção de dados (bit rot)?',
      badge: '💽 Storage',
      options: [
        'Compressão transparente',
        'Checksum de cada bloco combinado com redundância (self-healing)',
        'Deduplicação',
        'Quotas por dataset',
      ],
      correct: 1,
      explanation: 'O ZFS calcula um checksum de cada bloco. Na leitura, se o checksum não bate e existe redundância (mirror ou RAID-Z), ele recupera a cópia boa e reescreve o bloco corrompido — o self-healing. É o que combate o bit rot que passa despercebido em sistemas tradicionais.',
      trail: 'avancados',
    },
  // ── Banco de Dados (Sprint PILARES) ────────────────────────────────────────
  {
      text: 'Qual ferramenta faz o backup lógico de um banco PostgreSQL exportando-o como SQL?',
      badge: '🗄️ Banco de Dados',
      options: [
        'mysqldump',
        'pg_dump',
        'rsync',
      ...
      correct: 1,
      explanation: '`pg_dump` é o utilitário de backup lógico do PostgreSQL — gera um arquivo SQL (ou formato custom) que recria o banco. O equivalente no MariaDB/MySQL é o `mysqldump`. Backup lógico é portátil entre versões, diferente do físico (cópia de arquivos).',
      trail: 'avancados',
    },
  {
      text: 'Numa replicação primária-réplica, como a réplica se mantém sincronizada com a primária?',
      badge: '🗄️ Banco de Dados',
      options: [
        'Copia o diretório de dados inteiro a cada minuto',
        'Aplica continuamente o log de transações (WAL no PostgreSQL, binlog no MariaDB)',
        'Executa pg_dump na primária de hora em hora',
        'Compartilha o mesmo arquivo de dados via NFS',
      ],
      correct: 1,
      explanation: 'A primária registra cada mudança no log de transações — WAL (Write-Ahead Log) no PostgreSQL, binlog no MariaDB. A réplica recebe e reaplica esse stream em tempo real, ficando quase idêntica. Isso habilita leitura distribuída e failover rápido.',
      trail: 'avancados',
    },
  {
      text: 'Por que NÃO se deve usar o superusuário do banco (postgres/root) para a aplicação?',
      badge: '🗄️ Banco de Dados',
      options: [
        'Porque o superusuário é mais lento',
        'Pelo princípio do menor privilégio — a aplicação deve ter um usuário próprio com permissão só no seu schema/banco',
        'Porque o superusuário não aceita conexões TCP',
        'Porque o superusuário não suporta TLS',
      ],
      correct: 1,
      explanation: 'O superusuário pode apagar qualquer banco, alterar permissões e ler tudo. Se a aplicação (ou um SQL injection nela) usar essa conta, o estrago é total. Crie um usuário dedicado com `GRANT` apenas no banco/schema necessário — menor privilégio.',
      trail: 'avancados',
    },
  {
      text: 'Qual a diferença entre backup lógico (pg_dump/mysqldump) e backup físico no contexto de bancos de dados?',
      badge: '🗄️ Banco de Dados',
      options: [
        'O lógico exporta comandos SQL portáteis; o físico copia os arquivos de dados no disco',
        'O lógico é sempre mais rápido de restaurar',
        'O físico só funciona com o banco desligado, o lógico nunca',
        'Não há diferença prática entre eles',
      ],
      correct: 0,
      explanation: 'Backup lógico (pg_dump/mysqldump) gera SQL — portátil entre versões e máquinas, ideal para migrações, mas lento de restaurar em bancos grandes. Backup físico copia os arquivos do datadir (ex.: pg_basebackup) — rápido para bancos enormes, porém preso à mesma versão.',
      trail: 'avancados',
    },
  // ── Servidor de E-mail (Sprint PILARES) ────────────────────────────────────
  {
      text: 'No servidor de e-mail Linux clássico, qual a divisão de papéis entre Postfix e Dovecot?',
      badge: '📧 Mail',
      options: [
        'Postfix entrega ao cliente via IMAP; Dovecot envia via SMTP',
        'Postfix é o MTA (envia/recebe via SMTP); Dovecot entrega ao cliente via IMAP/POP3',
        'Ambos fazem exatamente a mesma coisa',
        'Postfix é o anti-spam; Dovecot é o webmail',
      ],
      correct: 1,
      explanation: 'Postfix é o MTA (Mail Transfer Agent): fala SMTP, recebe e roteia mensagens entre servidores. Dovecot é o MDA/servidor de acesso: entrega as mensagens ao cliente do usuário via IMAP ou POP3. Juntos cobrem o ciclo completo do correio.',
      trail: 'avancados',
    },
  {
      text: 'Qual a porta TCP padrão do protocolo SMTP usado pelo Postfix para receber e-mail entre servidores?',
      badge: '📧 Mail',
      options: [
        'Porta 110',
        'Porta 143',
      ...
      correct: 2,
      explanation: 'A porta 25 é o SMTP entre servidores (MTA-para-MTA). A 587 é o submission (cliente autenticado enviando), a 110 é POP3 e a 143 é IMAP. O Postfix escuta a 25 para receber correio de outros servidores na internet.',
      trail: 'avancados',
    },
  {
      text: 'Para que serve a autenticação SASL configurada no Postfix?',
      badge: '📧 Mail',
      options: [
        'Para criptografar o disco do servidor',
        'Para exigir que o cliente prove sua identidade antes de poder enviar e-mail — evitando open relay',
        'Para acelerar a entrega de mensagens',
        'Para indexar as mensagens recebidas',
      ],
      correct: 1,
      explanation: 'SASL (Simple Authentication and Security Layer) faz o Postfix exigir login/senha antes de aceitar envio de um cliente. Sem isso, o servidor vira um open relay — qualquer um na internet o usa para disparar spam, e ele acaba em blocklists.',
      trail: 'avancados',
    },
  {
      text: 'Como o SpamAssassin decide se uma mensagem é spam?',
      badge: '📧 Mail',
      options: [
        'Bloqueia todo e-mail de remetentes desconhecidos',
        'Aplica um conjunto de regras heurísticas e listas, somando uma pontuação — acima do limiar, marca como spam',
        'Verifica apenas se o assunto contém a palavra "spam"',
        'Recusa qualquer mensagem sem TLS',
      ],
      correct: 1,
      explanation: 'O SpamAssassin pontua cada mensagem: regras heurísticas (cabeçalhos suspeitos, padrões de texto), DNSBLs, checagem bayesiana e SPF/DKIM somam pontos. Se o total ultrapassa o limiar configurado (ex.: 5.0), a mensagem é marcada ou descartada como spam.',
      trail: 'avancados',
    },
  {
      text: 'O que o padrão IEEE 802.1Q faz num quadro Ethernet?',
      badge: '🌐 Redes L2/L3',
      options: [
        'Criptografa o conteúdo do quadro',
        'Insere uma tag de 4 bytes com o VLAN ID, permitindo separar domínios de broadcast no mesmo cabo',
        'Comprime o quadro para economizar banda',
        'Adiciona um checksum extra de integridade',
      ],
      correct: 1,
      explanation: 'O 802.1Q insere uma tag de 4 bytes contendo o VLAN ID (1–4094). É isso que permite a uma porta trunk transportar várias VLANs e a um servidor Linux ter sub-interfaces eth0.10, eth0.20 — cada uma num domínio de broadcast independente.',
      trail: 'avancados',
    },
  {
      text: 'No bonding Linux, qual modo entrega banda agregada mas exige LACP configurado no switch?',
      badge: '🌐 Redes L2/L3',
      options: [
      ...
      correct: 2,
      explanation: 'O modo 802.3ad (LACP) negocia o agrupamento dos links com o switch e entrega banda agregada — mas só sobe se o switch tiver o port-channel LACP configurado do outro lado. O active-backup funciona com qualquer switch, porém sem somar banda.',
      trail: 'avancados',
    },
  {
      text: 'Por que uma rota estática correta não é suficiente para o Linux encaminhar tráfego entre duas redes?',
      badge: '🌐 Redes L2/L3',
      options: [
        'A rota precisa ser registrada no DNS primeiro',
        'É preciso habilitar net.ipv4.ip_forward=1 — sem isso o kernel não encaminha pacotes de terceiros',
        'O Linux nunca encaminha pacotes, só roteadores físicos fazem isso',
        'A rota só funciona com IPv6',
      ],
      correct: 1,
      explanation: 'A rota define o caminho, mas por padrão o kernel descarta pacotes cujo destino não é ele mesmo. net.ipv4.ip_forward=1 (e net.ipv6.conf.all.forwarding=1 para IPv6) liga o encaminhamento — é o que transforma o host num roteador. Persista em /etc/sysctl.d/.',
      trail: 'avancados',
    },
  {
      text: 'Qual a diferença entre o endereço IPv6 fe80::/10 e um endereço 2000::/3?',
      badge: '🌐 Redes L2/L3',
      options: [
        'Não há diferença — são sinônimos',
        'fe80::/10 é link-local (só na própria rede, não roteável); 2000::/3 é global unicast (público e roteável)',
        'fe80::/10 é mais rápido que 2000::/3',
        '2000::/3 é privado e fe80::/10 é público',
      ],
      correct: 1,
      explanation: 'fe80::/10 é link-local: existe automaticamente em toda interface IPv6 e só vale no segmento local — não é roteável. 2000::/3 é o bloco global unicast: endereços públicos roteáveis na internet. O ULA fc00::/7 é o equivalente IPv6 do endereçamento privado.',
      trail: 'avancados',
    },
  {
      text: 'No keepalived/VRRP, o que é o VIP (Virtual IP)?',
      badge: '♻️ Alta Disponibilidade',
      options: [
        'O IP físico fixo do servidor principal',
        'Um endereço flutuante que não pertence a nenhum nó — é carregado por quem estiver como MASTER',
        'O IP do switch que conecta os nós',
        'Um endereço usado apenas para gerência via SSH',
      ],
      correct: 1,
      explanation: 'O VIP é um endereço que flutua: quem estiver no papel de MASTER o carrega na interface e responde por ele. Clientes e DNS apontam sempre para o VIP. Quando o MASTER cai, o VIP migra para outro nó (com um ARP gratuito) e o cliente nem percebe a troca de máquina.',
      trail: 'avancados',
    },
  {
      text: 'Por que um cluster Pacemaker de produção não deve rodar com STONITH desabilitado?',
      badge: '♻️ Alta Disponibilidade',
      options: [
        'STONITH só serve para acelerar o boot dos nós',
        'Sem fencing, uma rede partida pode deixar dois nós escrevendo no mesmo dado — corrupção (split-brain)',
        'Desabilitar STONITH viola a licença do Pacemaker',
        'STONITH é necessário apenas para clusters com mais de 10 nós',
      ],
      correct: 1,
      explanation: 'STONITH (Shoot The Other Node In The Head) é o fencing: garante que o nó perdido foi mesmo desligado antes de promover o recurso noutro lugar. Sem ele, um nó isolado mas vivo continua escrevendo enquanto o cluster sobe o mesmo recurso — corrupção de dados. Desligar STONITH só é aceitável em lab isolado.',
      trail: 'avancados',
    },
  {
      text: 'Por que clusters de produção preferem um número ímpar de nós (ou um qdevice)?',
      badge: '♻️ Alta Disponibilidade',
      options: [
        'Número ímpar economiza energia',
        'Para sempre haver maioria e evitar split-brain — num cluster de 2 nós isolados ninguém tem quorum',
        'Pacemaker só funciona com 3, 5 ou 7 nós',
        'É uma exigência do protocolo Corosync',
      ],
      correct: 1,
      explanation: 'Quorum exige maioria. Num cluster de 2 nós que se isolam, cada lado vê só 1 de 2 votos — ninguém forma maioria, ou pior, ambos se acham donos. Número ímpar de nós quebra o empate; quando só há 2 servidores reais, adiciona-se um qdevice (corosync-qnetd) como terceiro votante árbitro.',
      trail: 'avancados',
    },
  {
      text: 'No keepalived, para que serve um track_script com weight negativo?',
      badge: '♻️ Alta Disponibilidade',
      options: [
        'Para registrar logs detalhados do failover',
        'Para baixar a priority do MASTER quando o serviço local falha — disparando o failover por serviço, não só por queda do nó',
        'Para criptografar os anúncios VRRP',
        'Para limitar a banda usada pelo VRRP',
      ],
      correct: 1,
      explanation: 'Sem track_script o VRRP só detecta a queda do nó — não a do serviço. Com um vrrp_script que checa o serviço (ex.: systemctl is-active) e um weight negativo, ao falhar o healthcheck a priority do MASTER cai abaixo da do BACKUP, invertendo a eleição. O VIP migra mesmo com o host de pé.',
      trail: 'avancados',
    },
  {
      text: 'Na AWS, qual é o princípio fundamental ao criar políticas IAM?',
      badge: '☁️ Cloud',
      options: [
        'Dar permissão total (AdministratorAccess) a todos para evitar bloqueios',
        'Menor privilégio — conceder apenas as permissões estritamente necessárias para a tarefa',
        'Usar sempre a conta root para todas as operações',
        'Compartilhar uma única chave de acesso entre toda a equipe',
      ],
      correct: 1,
      explanation: 'O princípio do menor privilégio (least privilege) é a base do IAM: cada usuário, grupo ou role recebe somente as permissões necessárias. Isso limita o estrago de uma credencial vazada. A conta root nunca deve ser usada no dia a dia, e cada pessoa tem suas próprias credenciais.',
      trail: 'avancados',
    },
  {
      text: 'O que é uma VPC (Virtual Private Cloud) na AWS?',
      badge: '☁️ Cloud',
      options: [
        'Um tipo de instância de banco de dados gerenciado',
        'Uma rede virtual isolada e logicamente separada onde você provisiona seus recursos (EC2, etc.)',
        'Um serviço de armazenamento de objetos',
        'Uma ferramenta de monitoramento de custos',
      ],
      correct: 1,
      explanation: 'A VPC é a sua rede privada virtual na AWS — você define o bloco CIDR, cria sub-redes (públicas e privadas), route tables e gateways. É o equivalente na nuvem ao seu datacenter de rede: tudo que você lança (EC2, RDS) vive dentro de uma VPC.',
      trail: 'avancados',
    },
  {
      text: 'Qual a diferença entre um Security Group e uma Network ACL na AWS?',
      badge: '☁️ Cloud',
      options: [
        'Não há diferença — são sinônimos',
        'O Security Group é stateful e atua na instância; a Network ACL é stateless e atua na sub-rede',
        'O Security Group atua na sub-rede; a Network ACL atua na instância',
        'Ambos são stateless e atuam no mesmo nível',
      ],
      correct: 1,
      explanation: 'O Security Group é o firewall da instância EC2: stateful (tráfego de resposta é liberado automaticamente) e só tem regras de allow. A Network ACL é o firewall da sub-rede: stateless (precisa de regra de entrada E saída) e permite regras de deny. São camadas complementares.',
      trail: 'avancados',
    },
  {
      text: 'Por que se recomenda colocar servidores de aplicação em sub-redes privadas numa VPC?',
      badge: '☁️ Cloud',
      options: [
        'Sub-redes privadas são mais baratas',
        'Para que não tenham IP público nem rota direta da internet — reduzindo a superfície de ataque',
        'Sub-redes privadas têm mais largura de banda',
        'É uma exigência legal da AWS',
      ],
      correct: 1,
      explanation: 'Numa arquitetura segura, só o load balancer ou bastion fica na sub-rede pública. Servidores de aplicação e bancos ficam em sub-redes privadas — sem IP público e sem rota da internet. Eles acessam a internet (para updates) via NAT Gateway, mas ninguém de fora os alcança diretamente.',
      trail: 'avancados',
    },
  {
      text: 'No Git, quais são os três estados de um arquivo modificado?',
      badge: '🔀 Git',
      options: [
        'Aberto, fechado e salvo',
        'Modified (working directory), Staged (index) e Committed (repositório)',
        'Local, remoto e backup',
        'Público, privado e oculto',
      ],
      correct: 1,
      explanation: 'Git tem três áreas: o working directory (onde você edita — arquivo Modified), a staging area / index (onde git add coloca o que vai entrar no commit — Staged) e o repositório .git (onde git commit grava permanentemente — Committed). Entender esse fluxo é a base do Git.',
      trail: 'avancados',
    },
  {
      text: 'Qual a diferença entre git merge e git rebase ao integrar uma branch?',
      badge: '🔀 Git',
      options: [
        'Não há diferença prática',
        'O merge cria um commit de merge preservando o histórico; o rebase reaplica os commits por cima, gerando um histórico linear',
        'O rebase apaga a branch; o merge a mantém',
        'O merge só funciona com branches remotas',
      ],
      correct: 1,
      explanation: 'O merge une as duas linhas criando um commit de merge — o histórico mostra a ramificação real. O rebase pega os commits da sua branch e os reaplica em cima da base atualizada, produzindo um histórico linear e limpo. Regra de ouro: nunca faça rebase de commits já publicados/compartilhados.',
      trail: 'avancados',
    },
  {
      text: 'O que acontece quando o Git reporta um "merge conflict"?',
      badge: '🔀 Git',
      options: [
        'O repositório foi corrompido e precisa ser clonado de novo',
        'As duas branches alteraram a mesma região de um arquivo — você precisa resolver manualmente qual versão fica',
        'A branch remota foi deletada',
        'O commit anterior não tinha mensagem',
      ],
      correct: 1,
      explanation: 'Um conflito ocorre quando as duas branches modificaram as mesmas linhas de um arquivo — o Git não sabe qual escolher. Ele marca a região com <<<<<<<, ======= e >>>>>>>. Você edita o arquivo deixando a versão correta, faz git add e finaliza com git commit.',
      trail: 'avancados',
    },
  {
      text: 'O que é uma feature branch no fluxo colaborativo com Git?',
      badge: '🔀 Git',
      options: [
        'A branch principal de produção, sempre estável',
        'Uma branch curta e isolada criada para desenvolver uma funcionalidade, depois integrada via Pull Request',
        'Um backup automático do repositório',
        'A branch onde ficam apenas os arquivos de configuração',
      ],
      correct: 1,
      explanation: 'No feature branch workflow, cada nova funcionalidade ou correção é desenvolvida numa branch própria, isolada da main. Quando pronta, abre-se um Pull Request para revisão de código antes do merge. Isso mantém a main sempre estável e dá rastreabilidade a cada mudança.',
      trail: 'avancados',
    },
  {
      text: 'Qual é o objetivo de uma certificação como a LPIC-1 para a carreira em Linux?',
      badge: '🎖️ Carreira',
      options: [
        'Substituir completamente a experiência prática',
        'Validar de forma neutra (sem amarrar a um fornecedor) competências de administração de sistema Linux no nível júnior',
        'Garantir emprego automático em qualquer empresa',
        'Permitir acesso root a servidores de produção',
      ],
      correct: 1,
      explanation: 'A LPIC-1 (Linux Professional Institute) é uma certificação neutra de fornecedor: cobre administração de sistema em qualquer distribuição. Ela comprova conhecimento ao mercado e estrutura o estudo, mas não substitui a prática — currículo, portfólio e experiência continuam essenciais.',
      trail: 'avancados',
    },
  {
      text: 'Por que um portfólio técnico no GitHub é valioso numa candidatura de SysAdmin/DevOps?',
      badge: '🎖️ Carreira',
      options: [
        'Porque o GitHub paga os candidatos com mais repositórios',
        'Porque comprova competência com projetos reais — scripts, configs e labs que o recrutador pode inspecionar',
        'Porque substitui a necessidade de entrevista técnica',
        'Porque garante que o candidato sabe programar em qualquer linguagem',
      ],
      correct: 1,
      explanation: 'O portfólio mostra, não conta. Repositórios com scripts de automação, playbooks Ansible, configs de firewall comentadas e labs documentados dão ao recrutador evidência concreta de competência — algo que o currículo sozinho não consegue transmitir.',
      trail: 'avancados',
    },
  {
      text: 'Numa entrevista técnica de Linux, qual abordagem demonstra mais maturidade ao receber um problema de troubleshooting?',
      badge: '🎖️ Carreira',
      options: [
        'Chutar a primeira solução que vier à cabeça para responder rápido',
        'Seguir um método estruturado — isolar a camada (rede, serviço, app), verificar logs e explicar o raciocínio em voz alta',
        'Dizer que precisaria pesquisar no Google e não tentar',
        'Reiniciar o servidor imediatamente sem investigar',
      ],
      correct: 1,
      explanation: 'O entrevistador avalia o raciocínio, não só a resposta. Demonstrar um método (isolar a camada OSI, checar journalctl/ss/ping, formular hipóteses) e pensar em voz alta mostra maturidade de SysAdmin. Reiniciar às cegas ou chutar revela falta de processo.',
      trail: 'avancados',
    },
  {
      text: 'Por que documentar os labs do curso (firewall WAN/DMZ/LAN, cluster HA) num portfólio no GitHub fortalece a candidatura?',
      badge: '🎖️ Carreira',
      options: [
        'Porque aumenta o número de repositórios, e recrutadores contam repositórios',
        'Porque transforma conhecimento em prova concreta — um README com diagramas e comandos mostra que você sabe fazer, não só falar',
        'Porque o GitHub paga quem tem muitos projetos públicos',
        'Porque substitui a necessidade de certificações e entrevistas',
      ],
      correct: 1,
      explanation: 'Recrutadores de infraestrutura valorizam evidência prática. Um portfólio com READMEs bem escritos, diagramas de topologia e os comandos usados prova competência de forma verificável — complementa (não substitui) certificações e a entrevista, e dá assunto concreto para você explicar.',
      trail: 'avancados',
    },
  {
      text: 'No SELinux, qual é a diferença essencial entre o modelo de MAC dele e o do AppArmor?',
      badge: '🛡️ Segurança Avançada',
      options: [
        'Não há diferença — ambos usam o mesmo mecanismo',
        'SELinux baseia-se em labels (contextos) anexados a processos e arquivos no kernel; AppArmor baseia-se em caminhos (paths) absolutos definidos em perfil',
        'SELinux só funciona em RHEL e AppArmor só em Debian',
        'AppArmor é mais seguro porque é mais antigo',
      ],
      correct: 1,
      explanation: 'A diferença fundamental: SELinux ROTULA cada processo/arquivo com um contexto (user_u:role_r:type_t:level) gravado nos metadados do filesystem, e a política é por TIPO. AppArmor usa CAMINHO absoluto no perfil. Mover um arquivo no SELinux preserva a regra (o label viaja com o inode); no AppArmor a regra deixa de valer.',
      trail: 'avancados',
    },
  {
      text: 'Por que o backup do header LUKS é considerado obrigatório?',
      badge: '🛡️ Segurança Avançada',
      options: [
        'Para acelerar a montagem do volume',
        'Porque o header contém os key slots cifrados — se corromper sem backup, os dados ficam inacessíveis para sempre, mesmo com a passphrase correta',
        'Apenas por boa prática, mas não é crítico',
        'Para permitir múltiplas montagens simultâneas',
      ],
      correct: 1,
      explanation: 'O header LUKS guarda a chave-mestra do volume cifrada por cada passphrase (key slots). Se o header é sobrescrito (ex.: dd no setor errado, corrupção de disco) e você não tem backup feito com cryptsetup luksHeaderBackup, NENHUMA passphrase mais decifra — os dados estão criptograficamente perdidos. Por isso o backup do header é tão crítico quanto a passphrase.',
      trail: 'avancados',
    },
  {
      text: 'No auditd, qual regra registra TODA execução do binário /usr/bin/sudo?',
      badge: '🛡️ Segurança Avançada',
      options: [
        '-w /usr/bin/sudo -p wa -k sudo-watch',
  ...
  {
      text: 'Qual é o risco de rodar setenforce 0 em um servidor SELinux em produção?',
      badge: '🛡️ Segurança Avançada',
      options: [
        'Apenas o de ficar mais lento',
        'Qualquer comprometimento de aplicação passa a poder fazer o que o processo do app puder — a camada MAC do kernel para de bloquear acessos fora da política, virando um DAC tradicional',
        'O servidor reinicia automaticamente',
        'A passphrase do LUKS é apagada',
      ],
      correct: 1,
      explanation: 'setenforce 0 coloca o SELinux em modo Permissive: o kernel continua REGISTRANDO violações (AVC) mas NÃO bloqueia. Numa LFI/RCE em PHP, por exemplo, o processo do php-fpm passa a alcançar qualquer arquivo que seu uid possa ler — porque a camada MAC que o segurava só ao seu domínio (httpd_t) deixou de aplicar regras. Use Permissive apenas para diagnóstico, depois volte para Enforcing.',
      trail: 'avancados',
    },
  {
      text: 'Qual é a diferença fundamental entre Loki e Elasticsearch (ELK) para logs?',
      badge: '📊 Observabilidade',
      options: [
        'Não há diferença — ambos indexam tudo',
        'Loki indexa apenas LABELS (metadados) — barato e rápido para escalar; Elasticsearch indexa TODO o conteúdo (full-text search) — mais poderoso, mas RAM-hungry e caro',
        'Loki é proprietário e o ELK é open source',
        'Elasticsearch só funciona em Kubernetes',
      ],
      correct: 1,
      explanation: 'Esse é o trade-off arquitetural central. Loki (Grafana Labs) inspirou-se no modelo do Prometheus: indexa labels (job, app, severity) e armazena o conteúdo bruto. Resulta em armazenamento muito mais barato e escala horizontalmente fácil — mas a busca por palavras dentro do log é por força bruta (LogQL filtro |=). ELK (Elasticsearch) indexa cada token do log num inverted index: full-text search instantânea, agregações poderosas — pago em RAM e disco.',
      trail: 'avancados',
    },
  {
      text: 'No LogQL (linguagem de query do Loki), o que faz a expressão "rate({app=\\"api\\"} |= \\"panic\\" [5m])"?',
      badge: '📊 Observabilidade',
      options: [
        'Conta o número total de logs do app api',
        'Calcula a taxa de ocorrências por segundo da palavra "panic" nos logs do app api, na janela de 5 minutos — útil para alertas no Loki Ruler/Alertmanager',
        'Apaga todos os logs com a palavra panic',
        'Filtra logs de outros apps que não sejam api',
      ],
      correct: 1,
      explanation: 'Esse é o padrão para alertas baseados em logs. O seletor {app="api"} casa logs pelo label; |= "panic" filtra linhas com a palavra; rate(...[5m]) converte em taxa (eventos/segundo) sobre janela de 5 min — assim como em PromQL. Em alert rules do Loki Ruler: "expr: rate(...) > 0.1 → page on-call". É a mistura de PromQL com filtros de texto que torna o Loki tão poderoso para SRE.',
      trail: 'avancados',
    },
  {
      text: 'Por que um WAF (ModSecurity) protege contra ataques que o iptables/nftables não consegue parar?',
      badge: '🧱 WAF',
      options: [
        'WAF roda em hardware especial; iptables é só software',
        'iptables/nftables operam em L3/L4 (IP, porta, flags TCP) e não enxergam o payload HTTP. O WAF opera em L7: inspeciona URL, headers, body e cookies para detectar SQLi, XSS, LFI e o restante do OWASP Top 10',
        'Não há diferença prática',
        'O WAF é mais lento e por isso bloqueia mais',
      ],
      correct: 1,
      explanation: 'Esse é o ponto-chave. Um firewall L3/L4 vê "pacote TCP indo para a porta 443" — não vê a URL ?id=1 OR 1=1 que carrega um SQLi. O ModSecurity (ou o Coraza) intercepta a request HTTP DECIFRADA dentro do Nginx/Apache, separa headers/body/cookies em variáveis (ARGS, REQUEST_HEADERS) e aplica regras que reconhecem padrões de ataque — coisa que só dá pra fazer com visibilidade L7.',
      trail: 'avancados',
    },
  {
      text: 'No OWASP CRS (Core Rule Set), o que significa o paradigma de "anomaly scoring"?',
      badge: '🧱 WAF',
      options: [
        'Cada regra bloqueia imediatamente ao casar',
        'Cada regra que casa SOMA pontos a um placar; quando o placar ultrapassa o threshold (inbound_anomaly_score_threshold), aí sim a request é bloqueada — reduz falsos positivos',
        'É o nome dado ao paranoia level',
        'Bloqueia só anomalias detectadas por machine learning',
      ],
      correct: 1,
      explanation: 'O CRS evita o problema de "uma regra ingênua bloqueia tudo". Cada match SOMA um score (a maioria das regras dá 5 pontos para CRITICAL). A regra 949110 no fim verifica se o score total ultrapassou o threshold (5 padrão) e só aí dispara o deny. Assim, payloads ambíguos (que casam só uma regra fraca) passam, enquanto um ataque real casa várias regras e ultrapassa rapidamente. Combine com paranoia level (1-4) para subir o rigor.',
      trail: 'avancados',
    },
  {
      text: 'Qual é o papel do Promtail (ou do Vector) na stack Loki?',
      badge: '📊 Observabilidade',
      options: [
        'Substitui o Grafana como UI de visualização',
        'É o agente coletor instalado em cada host: lê arquivos de log, aplica pipeline_stages (regex, JSON, multiline) para extrair labels e envia para o Loki via push API',
        'Indexa o conteúdo dos logs igual o Logstash do ELK',
        'É um banco de dados auxiliar para queries lentas',
      ],
      correct: 1,
      explanation: 'O Promtail é o equivalente do Filebeat no mundo Loki — só que sem indexação. Ele faz tailing dos arquivos definidos em scrape_configs, anexa labels estáticos (job, host) e dinâmicos (extraídos via pipeline_stages com regex/JSON/labelmap), e empurra para o Loki via /loki/api/v1/push. Vector (Datadog) é uma alternativa moderna mais performática e poliglota (consome múltiplos sources, escreve em vários sinks).',
      trail: 'avancados',
    },
  {
      text: 'No ModSecurity, qual é a diferença entre DetectionOnly e On no SecRuleEngine?',
      badge: '🧱 WAF',
      options: [
        'DetectionOnly desativa todas as regras',
        'DetectionOnly apenas LOGA matches no audit log sem bloquear (modo "shadow"); On aplica a ação configurada (block/deny). DetectionOnly é o modo recomendado durante o tuning para mapear falsos positivos antes de ligar o bloqueio',
        'On é mais rápido porque pula a análise de regras',
        'Ambos bloqueiam — só muda o formato do log',
      ],
      correct: 1,
      explanation: 'Esse é o workflow operacional do WAF. Você liga DetectionOnly em produção primeiro, deixa rodar por dias coletando o que CASARIA (no audit log), revisa os falsos positivos (página de checkout legítima casando regra de SQLi por exemplo) e cria SecRuleRemoveById ou ctl:ruleRemoveTargetById. Só depois muda para On. Pular essa fase quase sempre quebra o site no dia 1.',
      trail: 'avancados',
    },
  {
      text: 'Qual a função do componente Kibana no stack ELK?',
      badge: '📊 Observabilidade',
      options: [
        'Coleta logs dos hosts (papel do Filebeat)',
        'É a interface web de visualização e análise: cria dashboards, executa queries KQL/Lucene contra o Elasticsearch e administra índices/ILM. Não armazena dados — só consome a API do Elasticsearch',
        'Faz parsing e enriquecimento dos logs (papel do Logstash)',
        'Substitui o Elasticsearch quando ele cai',
      ],
      correct: 1,
      explanation: 'Cada peça do ELK tem um papel claro: Beats/Filebeat coleta nos hosts, Logstash parseia/enriquece (grok, mutate), Elasticsearch indexa e armazena, Kibana visualiza. O Kibana é stateless: toda visualização que você cria é um documento .kibana no próprio Elasticsearch. Equivale ao Grafana no mundo Loki/Prometheus — só que mais opinado em logs.',
      trail: 'avancados',
    },
  {
      text: 'O que é "paranoia level" no OWASP CRS e qual o trade-off ao aumentar de 1 para 4?',
      badge: '🧱 WAF',
      options: [
        'É a versão do CRS instalado',
        'Controla quão agressivas são as regras: PL1 (padrão) gera poucos falsos positivos; PL4 ativa regras experimentais e estritas que pegam mais ataques mas exigem MUITO tuning. Trade-off direto: cobertura vs. falsos positivos',
        'Define quantos workers o ModSecurity usa',
        'É o número de IPs bloqueados por segundo',
      ],
      correct: 1,
      explanation: 'PL1 é o padrão e foi calibrado para sites genéricos rodarem sem grandes ajustes. PL2 já liga regras mais rigorosas (ex: nome de arquivo suspeito em parâmetro). PL3 e PL4 são para perfis high-security (banco, governo) — você precisa conhecer profundamente o tráfego legítimo da sua app para evitar quebrar o checkout. Aumentar paranoia level sem rodar antes em DetectionOnly é receita para downtime no dia 1.',
      trail: 'avancados',
    },
];
